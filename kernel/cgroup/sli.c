...
static const char * const schedlat_theshold_name[] = {
...
static const char * const memlat_threshold_name[] = {
...
static const char * const longterm_threshold_name[] = {
...
static int sli_event_inherit(struct cgroup *cgrp)
{
	...
	list_for_each_entry_rcu(event, &event_monitor->event_head, event_node) {
		...
		/*
		 * The event_type and event_id should not be observed before sli_event
...
static char *get_schedlat_name(enum sli_schedlat_stat_item sidx)
{
	char *name = NULL;
...
int sli_memlat_stat_show(struct seq_file *m, struct cgroup *cgrp)
{
	...
	if (!static_branch_likely(&sli_enabled)) {
		seq_puts(m, "sli is not enabled, please echo 1 > /proc/sli/sli_enabled\n");
		return 0;
	}
...
int sli_memlat_max_show(struct seq_file *m, struct cgroup *cgrp)
{
	...
	for (sidx = MEM_LAT_GLOBAL_DIRECT_RECLAIM; sidx < MEM_LAT_STAT_NR; sidx++) {
		...
		for_each_possible_cpu(cpu)
			latency_sum += per_cpu_ptr(cgrp->sli_memlat_stat_percpu,
							cpu)->latency_max[sidx];
...
int sli_schedlat_max_show(struct seq_file *m, struct cgroup *cgrp)
{
	...
	for (sidx = SCHEDLAT_WAIT; sidx < SCHEDLAT_STAT_NR; sidx++) {
		...
		for_each_possible_cpu(cpu)
			latency_sum += per_cpu_ptr(cgrp->sli_schedlat_stat_percpu,
							cpu)->latency_max[sidx];
...
static inline int sli_parse_threshold(char *buf, struct sli_event_control *sec)
{
	...
	/* Move the pointer to the position which after the delimiter */
	buf += (i + 1);
	len -= (i + 1);
...
static int sli_parse_parameter(char *buf, int len, struct sli_event_control *sec, int index)
{
	...
	case 3:
		...
		max_period = jiffies_to_usecs(MAX_JIFFY_OFFSET);
		if (value > max_period) {
			pr_err("period:%llu too large, limit max:%u\n", value, max_period);
			return -EINVAL;
		}
...
u32 sli_monitor_signal(struct cgroup *cgrp, struct sli_notify_event *notify_event)
{
	...
	if (waitqueue_active(&sctx->wqh)) // wait
...
static const struct proc_ops sli_enabled_fops = {
	.proc_open       = sli_enabled_open,
	.proc_read       = seq_read,
	.proc_write      = sli_enabled_write,
...
static int __init sli_proc_init(void)
{
	...
	if (!sli_workqueue) {
		pr_err("Create sli workqueue failed!\n");
		return -1;
	}
	proc_mkdir("sli", NULL);
